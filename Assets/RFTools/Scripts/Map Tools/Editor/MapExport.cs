...
public class MapExport {
	...
	public static bool BuildBundle(BuildTarget buildTarget, out string filepath, bool ignoreGraphCacheWarning) {
		...
		bool hasBuiltInGameMode = GameObject.FindObjectOfType<GameModeBase>() != null;
		bool usesTriggerSystem = GameObject.FindObjectOfType<Ravenfield.Trigger.TriggerBaseComponent>();
		...
		try {
			...
			build.assetBundleName = MetaDataUtils.FormatAssetBundleName(currentScene.name) + ".rfl";
		...
		if(!MetaDataUtils.ReadMetaData(currentScene.path, out metaData)) {
			metaData = MapMetaData.Default;
		}

		metaData.hasBuiltInGameMode = hasBuiltInGameMode;
		metaData.usesTriggerSystem = usesTriggerSystem;
