...
public class SanityCheck {
	...
	const string UNITY_ENGINE_MAJOR_VERSION = "2020.3";
	const string UNITY_ENGINE_RECOMMENDED_VERSION = "2020.3";
	...
	static bool DoSanityCheckVehicleMod(VehicleContentMod vehicleMod) {
		...
		if(vehicleMod.variants == null || vehicleMod.variants.Length == 0) {
			Log("Vehicle content mod " + vehicleMod.gameObject.name + " contains no vehicles.");
			return false;
		}
    ...
    static bool DoSanityCheckSkinMod(ActorSkinContentMod mod) {
        foreach(var skin in mod.skins) {
			...
			if(skin.rigSettings.version != ActorSkin.RigSettings.RECOMMENDED_VERSION) {
				Log("Custom Actor Skin " + skin.name + " uses outdated rig version " + skin.rigSettings.version + ", you probably want to use " + ActorSkin.RigSettings.RECOMMENDED_VERSION);
			}
        }
	...
	public static bool DoSanityCheck(bool ignoreGraphCacheWarning) {
		...
		bool unreadableMeshCollider = false;

		foreach(var mc in allMeshColliders) {
			if(mc.sharedMesh != null && !mc.sharedMesh.isReadable) {
				unreadableMeshCollider = true;
				Debug.LogError(string.Format("{1} collider mesh {0} is not readable and will be ignored when generating navmeshes. Please enable Read/Write in import settings.", mc.sharedMesh.name, mc.gameObject.name), mc.sharedMesh);
			}
		}

		if(unreadableMeshCollider) {
			Log("One or more mesh collider meshes are not readable and will be ignored when generating navmesh, please enable Read/Write in their model import settings. See console log for list of meshes.");
		}
	...
	public static bool CheckUnityEditorVersion() {
		...
		try {
			...
			string majorVersion = string.Format("{0}.{1}", split[0], split[1]);
	...
	static bool CameraHasSupportedScript(Camera camera) {
		return camera.GetComponent<SceneryCamera>() != null || camera.GetComponent<MinimapCamera>() != null;
	}
