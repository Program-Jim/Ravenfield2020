...
public class WorkshopWindow : EditorWindow {
	...
	const int MAX_ICON_FILE_SIZE_BYTES = 1000000;
	const uint APP_ID = 636480;

	const string APP_ID_PATH = "{0}steam_appid.txt";
	...
	bool InitializeSteamworks() {
		...
		InitializeWorkshopFiles();

		this.steamworks = new SteamworksWrapper();
		return this.steamworks.Initialize();
	}

	static void InitializeWorkshopFiles() {
		try {
			string projectPath = ProjectPath();
			string appIDPath = string.Format(APP_ID_PATH, projectPath);
			Debug.Log(appIDPath);
			if (!File.Exists(appIDPath)) {
				File.WriteAllText(appIDPath, APP_ID.ToString(), System.Text.Encoding.ASCII);
			}
	...
	void SetupItemTags() {
		...
		this.steamworks.currentItem.tags.Add("EA26 Compatible");
		...
		var maps = this.stagedContent.GetMaps();

		if (maps.Count > 0) {
			this.steamworks.currentItem.tags.Add("Maps");
			this.steamworks.currentItem.tags.Add("Maps - Mod Tools");

			foreach(var map in maps) {
				if(MetaDataUtils.ReadMetaData(map.FullName, out MapMetaData metaData)) {
					if(metaData.hasBuiltInGameMode) {
						this.steamworks.currentItem.tags.Add("Scripted Mission");
					}
					if(metaData.usesTriggerSystem) {
						this.steamworks.currentItem.tags.Add("Trigger System");
					}
				}
			}
		}
	...
	void StageExportedContent(FileInfo file) {
		...
		if(!this.stagedContent.ContainsFileOfName(file.Name)) {
			MetaDataUtils.CopyFileAndMetaData(file.FullName, CurrentItemStagingPath() + "/" + file.Name);
		}
	...
	void RemoveStagedContent(FileInfo file) {
		...
		if(!this.exportedContent.ContainsFileOfName(file.Name)) {
			...
			MetaDataUtils.CopyFileAndMetaData(file.FullName, ExportPath() + "/" + file.Name);
		}

		MetaDataUtils.DeleteFileAndMetaData(file.FullName);
	...
	public void FindModContent() {
		...
		foreach(FileInfo stagedFile in this.stagedContent.allContent.ToArray()) {
			FileInfo sourceFile;
			if(this.exportedContent.HasLocalVersionOfFile(stagedFile, out sourceFile)) {

				if (sourceFile.LastWriteTimeUtc > stagedFile.LastWriteTimeUtc) {
					// Staged file is outdated, update it.
					Debug.Log("Updating staged file: " + stagedFile.Name);
					FileInfo replacementFile = MetaDataUtils.CopyFileAndMetaData(sourceFile.FullName, stagedFile.FullName);

					this.stagedContent.MarkAsFileReplaced(stagedFile, replacementFile);
				}
				else {
					var sourceMetaDataInfo = new FileInfo(MetaDataUtils.GetMetaDataPath(sourceFile.FullName));
					var stagedMetaDataInfo = new FileInfo(MetaDataUtils.GetMetaDataPath(stagedFile.FullName));

					if(sourceMetaDataInfo.LastWriteTimeUtc > stagedMetaDataInfo.LastWriteTimeUtc) {
						// Only the meta data is outdated, update it
						MetaDataUtils.CopyMetaData(sourceFile.FullName, stagedFile.FullName);
						this.stagedContent.MarkAsUpdated(stagedFile);
					}
				}
			}
		}
